/**
 * Shared accent set used on category R cards (left bar) and partner chips.
 */
...
/** Text color only — for emails, labels, etc. */
...
export function accentIndex(key: string, offset = 0): number {
  let hash = 0;
...
export function accentTextClass(key: string, offset = 0): string {
  return ACCENT_TEXT[accentIndex(key, offset)];
}

export function accentChipClass(key: string, offset = 0): string {
