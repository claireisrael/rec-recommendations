...
/** Accent color for category badges / section headers on guest portal. */
...
export const DEFAULT_CATEGORY: RecommendationCategory = "clean_cooking";
...
/** Map REC25 report section titles → category keys. */
const SECTION_CATEGORY_MAP: { match: RegExp; category: RecommendationCategory }[] =
  [
    ...
    {
      match: /productive use of energy|productive use|pue/i,
      category: "agri_energy",
    },
...
export function resolveCategory(
  ...
  // Legacy Appwrite value — biofuels was removed from the REC category list
  if (normalized === "biofuels") return "agri_energy";
  if (
    RECOMMENDATION_CATEGORIES.includes(normalized as RecommendationCategory)
  ) {
    return normalized as RecommendationCategory;
  }
  // Comments from import: "Section: 3. Access To Finance"
  const section = normalized.replace(/^section:\s*/i, "");
  for (const entry of SECTION_CATEGORY_MAP) {
    if (entry.match.test(section)) return entry.category;
  }
  return DEFAULT_CATEGORY;
}

export function categoryFromSection(section: string): RecommendationCategory {
