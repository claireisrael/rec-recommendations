...
async function getDocument(cfg, id) {
...
async function resolveCodeForId(cfg, id, year) {
...
async function resolveAccessTarget(cfg, id) {
...
async function assertCanMutate(cfg, email, id) {
...
async function assertCanDelete(cfg, email, id) {
  const resolved = await resolveAccessTarget(cfg, id);
  if (!resolved.ok) return resolved;
